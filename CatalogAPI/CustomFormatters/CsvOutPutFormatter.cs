    ...
    public class CsvOutPutFormatter : TextOutputFormatter
    {
        ...
        public override async Task WriteResponseBodyAsync(OutputFormatterWriteContext context, Encoding selectedEncoding)
        {
            ...
            if(context.Object is CatalogItem)
            {
                ...
                buffer.Append($"{item.Id},{item.Name},{item.Price},{item.Quantity},{item.RecordLevel},{item.ManufacturingDate},{item.ImageUrl}" + Environment.NewLine);
            }
            else if (context.Object is IEnumerable<CatalogItem>)
            {
                var items = context.Object as IEnumerable<CatalogItem>;
                 buffer.Append("Id,Name,Price,Quantity,ReorderLevel,ManufacturingDate,ImageUrl");
